...
def save_embeddings_to_file(main_window: 'MainWindow', save_as=False):
    if not main_window.merged_embeddings:
        common_widget_actions.create_and_show_messagebox(main_window, '嵌入列表为空！', '没有可保存的嵌入', parent_widget=main_window)
        return
    ...
    if embedding_filename:
        with open(embedding_filename, 'w') as embed_file: #pylint: disable=unspecified-encoding
            ...
            common_widget_actions.create_and_show_toast_message(main_window, '嵌入已保存', f'嵌入已保存到文件：{embedding_filename}')
